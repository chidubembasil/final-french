...
function Activites() {
  ...
  // --- Handle Detail Fetch & JSON Parsing ---
  const handleOpenExercise = async (id: number) => {
    ...
    try {
      ...
      // Parsing Stringified JSON from API
  ...
  return (
    <main className="pt-20 bg-[#fcfaf8] min-h-screen">
      {/* Dynamic Hero Section - Restored to 90dvh */}
      ...
      {selectedExercise && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedExercise(null)} />
          
          <div className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[3rem] p-8 md:p-12 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
              ...
              {selectedExercise.showAnswerKey && (
                <button 
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase transition-all shadow-sm ${revealAnswers ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                >
            ...
            <button onClick={() => setSelectedExercise(null)} className="w-full mt-10 py-5 bg-slate-900 text-white rounded-[2rem] font-bold hover:bg-blue-600 transition-all shadow-xl">
              Complete Lesson
            </button>
          </div>
        </div>
