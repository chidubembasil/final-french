import { SplitSquareHorizontal, ArrowRight, PlayCircle, Book, PenTool, ChevronLeft, ChevronRight, CheckCircle2 } from "lucide-react";
      ...
      <section className="py-12 px-6 md:px-20 max-w-7xl mx-auto">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 mb-12 flex flex-col lg:flex-row gap-6 items-center">
          <div className="flex-1 w-full">
            ...
            <input 
              ...
              className="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none outline-none focus:ring-2 focus:ring-blue-600 transition-all"
              value={searchQuery}
          ...
          <div className="w-full lg:w-auto">
            ...
            <select 
              className="w-full lg:w-48 px-4 py-4 rounded-2xl bg-gray-50 border-none font-bold text-xs text-slate-700 outline-none"
              value={activeType}
            ...
            <div className="flex gap-2">
              {['All', ...DIFFICULTIES].map(d => (
                <button 
                  ...
                  className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${activeDifficulty === d ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                >
                  {d}
                </button>
              ))}
          ...
          {loading ? (
            ...
            currentExercises.map((item) => (
              <div key={item.id} className="group bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-2xl transition-all flex flex-col justify-between relative overflow-hidden">
                <div className={`absolute top-0 right-8 px-3 py-1 rounded-b-lg text-[8px] font-black uppercase text-white ${item.difficulty?.toLowerCase() === 'advanced' ? 'bg-red-600' : 'bg-blue-600'}`}>
                  {item.difficulty}
                </div>
                <div>
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-white ${item.exerciseType?.toLowerCase().includes('choice') ? 'bg-blue-600' : 'bg-red-600'}`}>
                    {getIcon(item.exerciseType)}
                  </div>
                  <h3 className="text-2xl font-bold text-slate-800 mb-1">{item.title}</h3>
                  <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-4">{item.exerciseType}</p>
                  <p className="text-gray-500 text-sm line-clamp-3 mb-6">{item.description}</p>
                </div>
                <button className="flex items-center gap-2 font-bold text-blue-700 group/btn">
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-3 rounded-xl border border-gray-200 hover:bg-gray-50 disabled:opacity-30"><ChevronLeft size={20} /></button>
            {[...Array(totalPages)].map((_, i) => (
              <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-12 h-12 rounded-xl font-bold transition-all ${currentPage === i + 1 ? 'bg-blue-700 text-white' : 'bg-white border border-gray-200 text-slate-600'}`}>{i + 1}</button>
            ))}
            <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-3 rounded-xl border border-gray-200 hover:bg-gray-50 disabled:opacity-30"><ChevronRight size={20} /></button>
          </div>
        )}
      </section>
    </main>
