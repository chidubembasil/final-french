...
function Activites() {
    ...
    fetch(`${CLIENT_KEY}api/galleries`)
      ...
      .then((data: GalleryHero[]) => {
        const matchingHero = data.find(
          (item) => item.purpose === "Other Page" && item.subPurpose === "Activities"
        );
  ...
  }, [CLIENT_KEY, activeType, activeDifficulty, searchQuery]);
  ...
  // Client-side search (fallback / additional filtering)
  ...
  const handleOpenExercise = async (id: number) => {
    ...
    try {
      ...
      const parsedExercise = {
        ...
        answerKey:
          typeof rawData.answerKey === "string" ? JSON.parse(rawData.answerKey) : rawData.answerKey,
        showAnswerKey:
          rawData.showAnswerKey === "true" ||
          rawData.showAnswerKey === true ||
          rawData.showAnswerKey === 1
      };
  ...
  const getIcon = (type: string) => {
    switch (type?.toLowerCase()) {
      case "multiple choice":
        return <PlayCircle size={32} />;
      case "gap filling":
        return <PenTool size={32} />;
      case "matching":
        return <SplitSquareHorizontal size={32} />;
      case "true or false":
  ...
  return (
    <main className="pt-20 bg-[#fcfaf8] min-h-screen">
      {/* Hero with lazy loading → actually eager because it's above the fold */}
        ...
        {loadingHero ? (
          ...
          <>
            <img
              ...
              loading="eager"           // Hero → important → eager loading
              decoding="async"
            />
      ...
      <section className="py-12 px-6 md:px-20 max-w-7xl mx-auto">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 mb-12 flex flex-col lg:flex-row gap-6 items-center">
          ...
          <div className="w-full lg:w-auto">
            ...
            <select
              className="w-full lg:w-48 px-4 py-4 rounded-2xl bg-gray-50 border-none font-bold text-xs cursor-pointer"
              value={activeType}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {loading ? (
            ...
            currentExercises.map((item) => (
              <div
                key={item.id}
                className="group bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-2xl transition-all flex flex-col justify-between relative overflow-hidden"
              >
                <div
                  className={`absolute top-0 right-8 px-3 py-1 rounded-b-lg text-[8px] font-black uppercase text-white z-10 ${
                    item.difficulty?.toLowerCase() === "advanced" ? "bg-red-600" : "bg-blue-600"
                  }`}
                >
                  {item.difficulty}
                </div>
                ...
                <div>
                  <div
                    className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-white transition-transform group-hover:scale-110 duration-500 ${
                      item.exerciseType?.toLowerCase().includes("choice") ? "bg-blue-600" : "bg-red-600"
                    }`}
                  >
                  ...
                  <p className="text-[10px] font-black text-blue-500 uppercase mb-4 tracking-widest">
                    {item.exerciseType}
                  </p>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3">
            <button
              ...
              className="p-4 rounded-2xl bg-white border border-gray-100 disabled:opacity-30 hover:bg-gray-50 transition-all"
            >
              <ChevronLeft size={20} />
            </button>
      ...
      {selectedExercise && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[3rem] p-8 md:p-12 shadow-2xl">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
              ...
              {selectedExercise.showAnswerKey && (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase transition-all shadow-sm ${
                    revealAnswers ? "bg-green-600 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                  }`}
                >
                ...
                return (
                  <div key={idx} className="p-8 bg-gray-50/50 rounded-[2.5rem] border border-gray-100">
                    <p className="font-bold text-xl text-slate-800 mb-6 flex gap-4">
                      <span className="text-blue-600 bg-blue-50 w-8 h-8 rounded-lg flex items-center justify-center text-sm shrink-0">
                        {idx + 1}
                      </span>
                      {q.questionText}
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {q.options?.map((opt: string, i: number) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`px-6 py-4 border-2 rounded-2xl text-sm font-bold transition-all ${
                              isCorrect
            ...
            <button
              onClick={() => setSelectedExercise(null)}
              className="w-full mt-10 py-5 bg-slate-900 text-white rounded-[2rem] font-bold hover:bg-blue-600 transition-all shadow-xl"
            >
              Complete Lesson
            </button>
          </div>
