  ...
  return (
    <main className="pt-20 bg-gray-50/50 min-h-screen relative">
      ...
      <div className="relative w-full h-[90dvh] overflow-hidden bg-slate-900">
        {loadingHero ? <div className="absolute inset-0 bg-slate-800 animate-pulse" /> : (
          <>
        ...
        <div className="flex flex-col gap-6 mb-12 bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search..." className="w-full pl-11 pr-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 outline-none focus:ring-2 focus:ring-blue-500" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <select className="px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 font-bold text-sm outline-none" value={topicFilter} onChange={(e) => setTopicFilter(e.target.value)}>
              ...
              {['Pronunciation', 'Grammar', 'Conversation', 'Culture', 'Basics'].map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            <select className="px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 font-bold text-sm outline-none" value={audienceFilter} onChange={(e) => setAudienceFilter(e.target.value)}>
              <option value="All">All Audiences</option>
              {['Beginners', 'Intermediate', 'Advanced', 'All'].map(a => <option key={a} value={a}>{a}</option>)}
            </select>
            <select className="px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 font-bold text-sm outline-none" value={stateFilter} onChange={(e) => setStateFilter(e.target.value)}>
              ...
              {["Lagos", "Abuja", "Rivers", "Oyo", "Kano", "Enugu"].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100">
             <div className="flex bg-gray-100 p-1 rounded-xl">
                {['All', 'Audio', 'Video'].map(m => (
                  <button key={m} onClick={() => setMediaFilter(m)} className={`px-5 py-1.5 rounded-lg text-xs font-bold transition-all ${mediaFilter === m ? 'bg-white text-blue-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{m}</button>
                ))}
             </div>
             <div className="flex gap-2">
                {['All', 'A1', 'A2', 'B1', 'B2'].map(lvl => (
                  <button key={lvl} onClick={() => setLevelFilter(lvl)} className={`px-4 py-1.5 rounded-full text-xs font-bold border transition-all ${levelFilter === lvl ? 'bg-blue-800 border-blue-800 text-white shadow-md' : 'bg-white border-gray-200 text-gray-600'}`}>{lvl}</button>
                ))}
             </div>
          </div>
        </div>
          ...
          <>
                ...
                return (
                  <div key={item.id} className="group bg-white rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all flex flex-col h-full overflow-hidden">
                    ...
                    <div className="p-8 flex flex-col flex-grow">
                      ...
                      <button onClick={() => setActivePodcast(item)} className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all">View Transcript</button>
                    </div>
                  </div>
                );
              })}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 mt-12">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-4 bg-white border border-gray-100 rounded-2xl disabled:opacity-20 hover:bg-gray-50 transition-all"><ChevronLeft size={20} /></button>
                <p className="text-xs font-black text-gray-400 uppercase">Page {currentPage} of {totalPages}</p>
                <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-4 bg-white border border-gray-100 rounded-2xl disabled:opacity-20 hover:bg-gray-50 transition-all"><ChevronRight size={20} /></button>
              </div>
            )}
          </>
        )}
